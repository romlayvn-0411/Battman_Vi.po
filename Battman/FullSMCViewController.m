...
- (instancetype)init {
	...
	UISearchController *scon=[UISearchController new];
	scon.searchResultsUpdater=(id)self;
	scon.delegate=(id)self;
	scon.obscuresBackgroundDuringPresentation=NO;
	self.navigationItem.searchController=scon;
...
- (void)didDismissSearchController:(UISearchController *)sc {
	_searchValue=NULL;
}

- (void)updateSearchResultsForSearchController:(UISearchController *)sc {
	_searchValue=sc.searchBar.text;
	[self.tableView reloadSections:[NSIndexSet indexSetWithIndex:1] withRowAnimation:UITableViewRowAnimationFade];
}

- (BOOL)_shouldHide:(NSIndexPath *)ip {
	if(_searchValue&&ip.section) {
		unsigned int key = htonl(allkeys[ip.row].key);
		if([_searchValue length]&&![[NSString stringWithFormat:@"%.4s",(char*)&key] localizedCaseInsensitiveContainsString:_searchValue])
			return YES;
	}
	return NO;
}
...
- (CGFloat)tableView:(UITableView *)tv heightForRowAtIndexPath:(NSIndexPath *)ip {
	if([self _shouldHide:ip])
		return 0;
	return [super tableView:tv heightForRowAtIndexPath:ip];
}
